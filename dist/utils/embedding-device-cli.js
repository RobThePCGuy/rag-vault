const DEVICE_VALUE_FLAGS = ['--embedding-device', '--device'];
const AUTO_DEVICE_FLAGS = ['--gpu-auto'];
function setEmbeddingDevice(value, source) {
...
function parseInlineFlagValue(arg, prefix) {
 ...
 * - --gpu-auto
 */
export function applyEmbeddingDeviceCliOverride(args) {
    const remaining = [];
    for (let i = 0; i < args.length; i++) {
        ...
        if (AUTO_DEVICE_FLAGS.includes(arg)) {
        ...
        if (DEVICE_VALUE_FLAGS.includes(arg)) {
...
//# sourceMappingURL=embedding-device-cli.js.map