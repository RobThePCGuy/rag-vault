 ...
 */
export function createConfigRouter(dbManager) {
    ...
    router.post('/databases/switch', asyncHandler(async (req, res) => {
    ...
    router.post('/databases/create', asyncHandler(async (req, res) => {
    ...
    router.post('/databases/scan', asyncHandler(async (req, res) => {
    ...
    router.delete('/databases', asyncHandler(async (req, res) => {
    ...
    router.get('/allowed-roots', asyncHandler(async (_req, res) => {
        const info = await dbManager.getAllowedRootsInfo();
        res.json(info);
    }));
    // POST /api/v1/config/allowed-roots - Add a new allowed root
    router.post('/allowed-roots', asyncHandler(async (req, res) => {
    ...
    router.delete('/allowed-roots', asyncHandler(async (req, res) => {
        const { path: rootPath } = req.body;
        if (!rootPath || typeof rootPath !== 'string') {
            throw new ValidationError('path is required and must be a string');
        }
    ...
    router.get('/hybrid-weight', asyncHandler(async (_req, res) => {
        const weight = dbManager.getHybridWeight();
        res.json({ weight });
    }));
    // PUT /api/v1/config/hybrid-weight - Set hybrid search weight
    router.put('/hybrid-weight', asyncHandler(async (req, res) => {
