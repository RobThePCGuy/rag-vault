...
describe('Database Switching E2E Workflow', () => {
  ...
  it('User Journey: Create database A → Add documents → Create database B → Add documents → Switch between them → Verify isolation', async () => {
    ...
    try {
      ...
      const dogsInA = resultsA2.filter((r: { filePath: string }) => r.filePath.includes('dogs-doc'))
      ...
      const catsInB = resultsB2.filter((r: { filePath: string }) => r.filePath.includes('cats-doc'))
